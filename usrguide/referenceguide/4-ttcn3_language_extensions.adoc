...
* The result of logging an instance of a class contains the class name and the logging results of the superclass (if there is one) and supertraits (if there are any) in round brackets, separated by commas.
...
* A definition in the subclass can only have the same name as a definition in one of the superclasses or supertraits, if both are methods (incl. external or abstract), and both have the same prototype (i.e. identical return type, identical formal parameter types and direction).
...
* Only one object is created when instanciating an `external` class, there are no separate internal or external objects. The code generated for an `external` class is written into a separate pair of source and header files. This code contains both the internal and external parts of the class. It's up to the user to implement any external functions, initialize the external part of the object in the constructor, add additional members and methods, or change existing ones. It's also up to the user whether the external or the internal part of the object is initialized first in the constructor.
* Internal classes cannot have `external` methods.
* The `external` parameters of a constructor cannot be referenced inside the constructor body in TTCN-3 code. They can only be used in the `external` class implementation in {cpp}.
...
Titan also supports trait classes and multiple inheritance as described in version 1.3.1 of the standard extension, with the clarification, that trait classes do not extend the built-in class `object`, only normal classes do. Trait classes also cannot be `external`.
...
* Default values of class method parameters cannot contain references to class members.
* The second parameter of `string2ttcn` cannot be of class type or of a type containing a class.
